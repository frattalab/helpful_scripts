# reads in a folder with TE counts from the Hammel TETranscripts tools
create_te_count_table = function(feature_count_folder, suffix = ".Aligned.sorted.out.bam",prefix = ""){
    library(data.table)
    ...
    selector_list = c("gene.TE",names(wide_feature_counts)[grepl("^X",names(wide_feature_counts))])
